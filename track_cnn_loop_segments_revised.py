...
def worker(fname,opts):
  ...
  #  fnp = rgi((phi,theta,r,[0,1]),vpt*2*np.pi/r.reshape(1,1,len(r),1)/np.sin(theta.reshape(1,len(theta),1,1)))
    fnp = rgi((theta,r,),np.mean(vp,axis=0)*2*np.pi/r.reshape(1,len(r))/np.sin(theta.reshape(len(theta),1))) #### is this even correct???? xxx
    Dt = dt*(int(fname[1])-int(fname[0]))

    merged_structures = np.load('{:s}{:s}merged_loops_f{:s}.npy'.format(datadir,fname_pref,fname[0]),allow_pickle=True)
    next_merged_structures = np.load('{:s}{:s}merged_loops_f{:s}.npy'.format(datadir,fname_pref,fname[1]),allow_pickle=True)
